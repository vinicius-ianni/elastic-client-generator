...
public class ActionStatus  implements XContentable<ActionStatus> {
  ...
  static {
    PARSER.declareObject(ActionStatus::setAck, (p, t) -> AcknowledgeState.PARSER.apply(p, null), ACK);
    PARSER.declareObject(ActionStatus::setLastExecution, (p, t) -> ExecutionState.PARSER.apply(p, null), LAST_EXECUTION);
    PARSER.declareObject(ActionStatus::setLastSuccessfulExecution, (p, t) -> ExecutionState.PARSER.apply(p, null), LAST_SUCCESSFUL_EXECUTION);
    PARSER.declareObject(ActionStatus::setLastThrottle, (p, t) -> ThrottleState.PARSER.apply(p, null), LAST_THROTTLE);
  }
