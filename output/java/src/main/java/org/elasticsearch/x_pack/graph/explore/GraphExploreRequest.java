...
public class GraphExploreRequest  implements XContentable<GraphExploreRequest> {
  ...
  static {
    PARSER.declareRouting(GraphExploreRequest::setRouting, (p, t) -> Routing.createFrom(p), ROUTING);
    PARSER.declareObject(GraphExploreRequest::setTimeout, (p, t) -> Time.PARSER.apply(p, null), TIMEOUT);
    PARSER.declareObject(GraphExploreRequest::setConnections, (p, t) -> Hop.PARSER.apply(p, null), CONNECTIONS);
    PARSER.declareObject(GraphExploreRequest::setControls, (p, t) -> GraphExploreControls.PARSER.apply(p, null), CONTROLS);
    PARSER.declareObject(GraphExploreRequest::setQuery, (p, t) -> QueryContainer.PARSER.apply(p, null), QUERY);
    PARSER.declareObjectArray(GraphExploreRequest::setVertices, (p, t) -> GraphVertexDefinition.PARSER.apply(p), VERTICES);
  }
